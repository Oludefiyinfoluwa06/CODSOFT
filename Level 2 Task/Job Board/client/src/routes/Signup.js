...
const Signup = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (
            ...
            setError("Input fields with * cannot be empty");
        } else if (firstname.length <= 3) {
            setError("Firstname length must be more than 3 characters");
        } else if (lastname.length <= 3) {
            setError("Lastname length must be more than 3 characters");
        } else if (!email.match( /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/)) {
            setError("Input a valid email address");
        } else if (password.length < 6) {
            setError("Password length must not be less than 6 characters long");
        } else {
            try {
                await axios.post('http://localhost:5000/jobboard/signup', { firstname, lastname, email, username, password, industry, jobTitle, experience, pjt, city, state, country })
                    .then(res => {
                        console.log(res);
                    ...
                    .catch(err => {
                        console.log(err);
                        setError("An error occured while trying to signup")
                    });
